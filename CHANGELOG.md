...
- **FAQ doc + in-app link** — [`docs/FAQ.md`](docs/FAQ.md) collects answers to specific "what is X" questions (pipeline canvas, webhook triggers, upstream dependencies + cycle detection). Linked from the in-app Help drawer via a new **FAQ ↗** link in its header, served raw through the existing `GET /api/docs/<file>` endpoint. ([`frontend/src/components/shared/HelpDrawer.tsx`](frontend/src/components/shared/HelpDrawer.tsx))

- **Consolidated pipeline "Triggers" card** — Schedule (previously a field inside Details), Upstream Dependencies, and Webhook / API Trigger were three separate cards stacked on the pipeline editor, all really answering "what starts this pipeline?" Replaced with one `TriggersCard.tsx` card with a Schedule / Dependencies / Webhook tab switcher; `DependenciesCard.tsx` and `WebhookCard.tsx` gained a `bare` prop so their existing logic renders inside the new tabs instead of duplicating it. Steps now appear higher on the page since there's one card instead of three above them. ([`frontend/src/components/pipeline/TriggersCard.tsx`](frontend/src/components/pipeline/TriggersCard.tsx), [`frontend/src/pages/PipelineEdit.tsx`](frontend/src/pages/PipelineEdit.tsx))
- **Password reset token pruning** — `ff_password_reset_tokens` rows were never deleted after expiry (the reset flow only checked `expires_at`/`used_at` logically, the row stayed forever). Added `_prune_expired_password_reset_tokens()` to the scheduler's existing daily cleanup job, mirroring the pattern already used for `ff_token_blocklist`. Webhook tokens (`ff_webhook_tokens`) were deliberately left unpruned — they're documented as never expiring by default, so auto-deleting them would silently break external integrations; revoking one already deletes its row immediately. ([`flowforge/engine/scheduler.py`](flowforge/engine/scheduler.py))
- **Gemini API as a third `ai_analyze` provider** — alongside Ollama (default) and Claude, the `ai_analyze` step now accepts `provider: "gemini"`, calling the Gemini API directly over REST (no SDK dependency, unlike `claude` which needs `pip install anthropic`) with the key sent as a header rather than a URL query param. Configure via `GEMINI_API_KEY` (has a free tier — [aistudio.google.com/apikey](https://aistudio.google.com/apikey)) and optionally `GEMINI_QUERY_MODEL` (default `gemini-2.5-flash`). ([`flowforge/steps/ai_analyze.py`](flowforge/steps/ai_analyze.py))
- **Claude/Gemini fallback for the Ollama-only AI utility endpoints** — SQL Explain, SQL Optimize, Data Profiler, Chart Generator, and Pipeline Failure Diagnosis (`flowforge/api/routes/ai.py`) now try Ollama first, then fall back to Claude and then Gemini (whichever is configured via `ANTHROPIC_API_KEY` / `GEMINI_API_KEY`) if Ollama is unreachable, via a new `_generate_text()` wrapper that reuses `_call_claude`/`_call_gemini` from `ai_analyze.py`. If a configured provider itself errors (e.g. `anthropic` not installed), the next candidate is tried before giving up; if none succeed the original "Ollama not reachable" error is preserved. `GET /setup/status` and the Settings → AI tab's `AI Features` card were extended to explain and surface this. ([`flowforge/api/routes/ai.py`](flowforge/api/routes/ai.py))
- **Settings page: dedicated AI tab** — the AI Features card moved out of the combined "Email & AI" tab into its own tab (`Account | Email | AI | System | Docs`), since it no longer only concerns email-adjacent OAuth setup. ([`frontend/src/pages/Settings.tsx`](frontend/src/pages/Settings.tsx))
- **Editable data retention settings** — the three retention values (pipeline run history, audit log, output file TTL) were env-var-only, requiring a `.env` edit and process restart to change; Settings → System only displayed two of them, read-only. Added a DB-backed override: `ff_system_settings` singleton table (migration `0029_system_settings`), a `flowforge/engine/settings.py` resolver that checks it first and falls back to the exact existing env-var logic when unset, and `GET`/`PUT /api/settings/retention` (admin-only writes). `Settings.tsx`'s `RetentionCard` is now an editable form for admins (read-only badges, now showing all three values, for everyone else) with a "Use default" reset per field. Output file TTL can't be set to `0` here — unlike the other two, `0` means "delete every file immediately," not "keep forever" — the `flowforge cleanup --days 0` CLI command still allows it, now gated behind a `click.confirm()` prompt (`--yes` to skip). Changes take effect on the next scheduled cleanup run, no restart needed. ([`flowforge/api/routes/settings.py`](flowforge/api/routes/settings.py), [`flowforge/engine/settings.py`](flowforge/engine/settings.py), [`frontend/src/pages/Settings.tsx`](frontend/src/pages/Settings.tsx))

### Fixed — 2026-07-09

- **Help drawer showed the wrong content on the pipeline editor** — `PipelineEdit.tsx` never passed a `helpTopic` to `TopBar`, so `useHelp`'s default silently fell back to the `dashboard` topic; opening Help while editing a pipeline showed the Dashboard intro card instead of anything pipeline-related. Compounded by `HelpDrawer.tsx`'s `topic === 'pipeline_builder'` branch (the step-type reference list) being unreachable dead code, since no page ever set that topic string. Now `PipelineEdit.tsx` passes `helpTopic="pipeline_builder"`, a matching `INTRO_CARDS.pipeline_builder` entry was added, and `STEP_TYPES` was extended with `data_load`/`bulk_load` — both already had written `STEP_HINTS` content that was never surfaced anywhere in the UI. ([`frontend/src/pages/PipelineEdit.tsx`](frontend/src/pages/PipelineEdit.tsx), [`frontend/src/lib/helpContent.ts`](frontend/src/lib/helpContent.ts), [`frontend/src/components/shared/HelpDrawer.tsx`](frontend/src/components/shared/HelpDrawer.tsx))
- **`docs/RUNBOOK.md` understated the scheduler's daily cleanup job** — it described output-file cleanup as something you'd need to "add as a cron job," when it's already automatic, and never documented run-history or audit-log pruning at all. Added §8a "Data Retention & Cleanup" covering every pruning task, its env var, and the "requires the scheduler process to be running" caveat; also brought `docs/data-flow.md`'s retention table in sync. ([`docs/RUNBOOK.md`](docs/RUNBOOK.md), [`docs/data-flow.md`](docs/data-flow.md))
