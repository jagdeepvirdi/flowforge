...
- **Step-level DAG dependencies** — draw real dependency edges between steps on the pipeline canvas for non-linear execution; a failed `stop` step only halts its own descendants (independent branches keep running), and `{{ steps.* }}` context is scoped to a step's transitive ancestors. A pipeline with no drawn edges runs through the original sequential/parallel-group engine, untouched
