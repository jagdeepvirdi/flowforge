...
- API rate limits (`@limiter.limit(...)`, e.g. the `10 per minute` cap on triggering a run) have the identical failure mode: flask-limiter defaults to per-process in-memory counters. When `FLOWFORGE_REDIS_URL` is set, `create_app()` points flask-limiter at the same Redis instance (`RATELIMIT_STORAGE_URI`), so a client's request count is shared across every Gunicorn worker instead of resetting per-worker. Requires the `redis` Python package — already pulled in by `flowforge-io[celery]`/`[all]`, or install standalone via `flowforge-io[redis]` if you want Redis for concurrency/rate-limiting without the Celery task queue.
